...
<ParamField path="type" type="DnsRecordType">
  DNS record type: `A`, `AAAA`, `TXT`, `CNAME`, `MX`, `NS`, `SOA`, `PTR`, `SRV`, `NAPTR`, `DS`, `DNSKEY`, `CAA`, etc.
  
  If not specified, the behavior depends on the resolver. The `node-dns` resolver will query multiple types (A, AAAA, CNAME), while DNS-over-HTTPS resolvers may default to A records.
</ParamField>
